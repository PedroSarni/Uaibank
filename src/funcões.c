...
void registrar_transferencia(FILE *arq_trans, int id_remetente, int id_destinatario, float valor){
    ...
    char data_hora[20];
    strftime(data_hora, sizeof(data_hora), "%d/%m/%Y %H:%M:%S", tm_local);

    fprintf(arq_trans, "%d,%d,%.2f,%s\n", id_remetente, id_destinatario, valor, data_hora);
    printf("Dados da transferência foram adicionados no arquivo de transferências.\n\n");
}
...
void salvar_encerrar(pessoa *banco, int usuarios){
    ...
    fprintf(arq,"ID,Nome,Sexo,Idade,Estado,Email,Telefone,Saldo\n");
    for(int i = 0; i < usuarios; i++){
